...
void UI_PauseVolumeControl::Reset()
{
	m_CurrentIndex = 0;
	m_VolumeBarOutline.SetPosition(m_VolumeBars->GetPosition());
}

void UI_PauseVolumeControl::InitialiseUI(std::string& fp, unsigned index)
{
	...
	for (unsigned i(0); i < MAX_OPTION_MENU_ENTRIES; ++i)
	{
		...
		m_VolumeNameFrames[i].SetLayerDepth(DRAW_DEPTH_UI_2);
	}
...
void UI_PauseVolumeControl::Render(DirectX::SpriteBatch& sb, DirectX::SpriteFont& sf, DirectX::DescriptorHeap* heap)
{
...
void UI_PauseVolumeControl::Update()
{
	//Set the scales for each of the volume bars relative to the current volume they represent
	//Master
	m_VolumeBars[0].SetScale(
		Game::GetGame()->GetAudioManager().GetVolumeControls().GetMasterVolume() / 1,
		m_VolumeBars[0].GetScale().y
	);
	//Music
	m_VolumeBars[1].SetScale(
		Game::GetGame()->GetAudioManager().GetVolumeControls().GetMusicVolume() / 1,
		m_VolumeBars[1].GetScale().y
	);
	//SFX
	m_VolumeBars[2].SetScale(
		Game::GetGame()->GetAudioManager().GetVolumeControls().GetSFXVolume() / 1,
		m_VolumeBars[2].GetScale().y
	);
...
UI_PauseVolumeControl::Actions UI_PauseVolumeControl::ProcessInput(char key)
{
	switch (key)
	{
	case (char)CS_Survivor_00::DMP::BACK:
		Game::GetGame()->GetGameplayManager().GetStateManager().SetActiveGameState(SM_GameStateManager::AGS::PAUSE_SCREEN_OPEN);
		break;
...
void UI_PauseVolumeControl::DecrementVolumes()
{
	if (m_CurrentIndex == 0)
	{
...
void UI_PauseVolumeControl::IncrementVolumes()
{
	if (m_CurrentIndex == 0)
	{
		Game::GetGame()->GetAudioManager().GetVolumeControls().IncMasterVolume(0.1f);
